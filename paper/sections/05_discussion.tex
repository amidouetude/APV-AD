...
Konya (GHI = 1\,650\,kWh\,m$^{-2}$) --- is one of the key mechanistic
findings of this study and requires careful interpretation before
deployment implications can be drawn.
...
identified as an important extension of the present framework.
...
a universal welfare measure with a single objectively correct weighting.
...
\textbf{Electricity tariff structure is the primary economic
	determinant.} The largest revenue stream at all four sites is PV
...
\textbf{The carbon credit scenario (S6) provides the strongest
	economic uplift in this study and is worth prioritising in policy
	design.} S6 raises
...
\textbf{Ouagadougou provides the strongest investment case among the
...
the APV--AD model at Ouagadougou-type sites offers a notable
...
achieve. This is a stronger result than the PV-only LCOE metric itself
suggests: it is a single-output comparison that ignores crop
production, water savings, and biogas, none of which are credited
here. A full value-stack LCOE that credits all outputs would likely
be lower than the PV-only LCOE at every site, and because the
PV-only LCOE already clears the utility-scale benchmark everywhere,
the full value-stack LCOE would likely remain below that benchmark
at all four sites.
...
relationship. A multi-crop sensitivity analysis --- requiring only a change of
the $PAR_{\text{sat}}$ parameter in the existing model --- is an
important next extension, as it would help determine how broadly
the PAR-saturation finding generalises beyond tomato.
...
and an important direction for experimental field
